...
def process_session(paths, config):
    ...
    if qr_path and os.path.exists(qr_path):
        ...
        img = cv2.imread(qr_path)
        img_resized = cv2.resize(img, FIXED_SIZE, interpolation=cv2.INTER_NEAREST)

        win_name = f"FBTI QR - {bti_combination}"
        cv2.namedWindow(win_name, cv2.WINDOW_NORMAL)
        cv2.resizeWindow(win_name, FIXED_SIZE[0], FIXED_SIZE[1])

        # 화면 해상도 (예: 1920x1080)
        screen_w, screen_h = 2560, 1600
        x_pos = (screen_w - FIXED_SIZE[0]) // 2
        y_pos = (screen_h - FIXED_SIZE[1]) // 2
        cv2.moveWindow(win_name, x_pos, y_pos)  # 창 위치 중앙으로 이동
